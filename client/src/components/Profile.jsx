...
const RenderUser = (user) => (
  <>
    <div className="wrap summary">
      <div>
        <span className="initials">{user.username.slice(0, 2)}</span>
      </div>
      <div>
        <h2>{user.username}</h2>
        <h2> {user.email}</h2>
...
const RenderInfo = ({ data }) => (
  <div className="card">
    {data.likesTitle != '' ? (
      <p>
        <strong>Title you liked: </strong>
        {data.likesTitle}
      </p>
    ) : null}
    {data.likesText != '' ? (
      <p>
        <strong>Text you liked: </strong>
        {data.likesText}
      </p>
    ) : null}
    {data.likesAuthor != '' ? (
    ...
    {String(data.links) != '' ? (
      <ul>
        <strong>Links:</strong>
        {Object.keys(data.links).map((item, index) => (
          <li key={index}>
            <a href={data.links[item]}>{data.links[item]}</a>
          </li>
        ))}
      </ul>
    ) : null}
...
const Summary = ({ data }) => {
  let newTags = () => {
    let tags = String(
      Object.keys(data)
        .map((item) => data[item].tags)
        .join(',')
    ).split(',');
    let cache = [];
    for (let i = 0; i < tags.length; i++) {
      if (tags[i] !== '' && cache.indexOf(tags[i]) == -1)
        cache += [tags[i]] + ' ';
    }
    return cache;
  };
  ...
  let newTitles = () => {
    let cat = String(
      Object.keys(data)
        .map((item) => data[item].likesTitle)
        .join(',')
    ).split(',');
    let cache = [];
    for (let i = 0; i < cat.length; i++) {
      if (cat[i] !== '' && cache.indexOf(cat[i]) == -1)
        cache += `${[cat[i]]} , `;
    }
    return cache;
  };
    ...
    <>
      <div>
        ...
        <div className="summary-p">
          <div>
            You have liked <strong>{Object.keys(data).length - 1}</strong>{' '}
            items.
          </div>
          <div className="line">
            Your favorite tags:{' '}
          ...
          <div className="line">
            <strong>Book titles you liked:</strong>
          </div>
          ...
          <div className="maxW">
            ...
            <div className="cardWrap">
              {Object.keys(data).map((item, index) => {
                return <RenderInfo key={index} data={data[item]} />;
              })}
            </div>
          </div>
        ) : (
